...
import { createPendingUserRegistration, verifyPendingUserRegistration, completePendingRegistration, resendPendingVerification, verifyCurrentPassword, loginUser } from "@/services/auth.service";
export interface ActionResult { success: boolean; message?: string; errors?: Record<string, string[]>; }
// Processes initial sign-up form submissions and initiates email verification
export async function signUpAction(
  prevState: ActionResult, formData: FormData): Promise<ActionResult> {
  const parsed = signUpSchema.safeParse({
    firstName: formData.get("firstName"), middleName: formData.get("middleName"),
    lastName: formData.get("lastName"), suffix: formData.get("suffix"),
    birthdate: formData.get("birthdate"), sex: formData.get("sex"),
    email: formData.get("email"), role: formData.get("role") });
  if (!parsed.success) return { success: false, message:
    "Please correct the highlighted fields.", errors: parsed.error.flatten().fieldErrors };
  try {
...
export async function verifyEmailAction(email: string, code: string): Promise<ActionResult> {
  try {
    await verifyPendingUserRegistration(email, code);
...
// Requests a new verification code email for pending registrations
...
export async function createPasswordAction(
  email: string, password: string, confirmPassword: string): Promise<ActionResult> {
...
export async function loginAction(
  prevState: ActionResult, formData: FormData): Promise<ActionResult & { redirectTo?: string }> {
  ...
  try {
    ...
    switch (user.role) {
      case "SUPER_ADMIN": redirectTo = "/admin"; break;
      case "COUNCIL_ADMIN": redirectTo = "/admin/login"; break;
      default: redirectTo = "/scout"; break;
    }
