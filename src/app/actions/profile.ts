...
interface ProfileFormData extends Omit<UpdateProfileInput, "userId"> {}
// Updates user profile information and revalidates profile path
export async function updateProfileAction(data: ProfileFormData) {
  ...
  try {
    const result = await updateProfile({ userId: currentUser.id, ...data });
    if (!result.success) return { success: false, message: result.message ?? "Unable to update profile." };
