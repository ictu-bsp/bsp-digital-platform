  ...
  return (
    <div className="bg-white rounded-2xl shadow p-6 text-zinc-900 flex flex-col gap-4">
      ...
      {breakdown && breakdown.length > 0 && (
        <div>
          {breakdownLabel && <p className="text-sm font-medium text-zinc-700 mb-2">
            {breakdownLabel}</p>}
          <ul className="flex flex-col gap-1.5">
            {breakdown.map((row) => (
              <li key={row.label} className="flex items-center justify-between
              text-sm border-b border-zinc-100 pb-1.5 last:border-0">
      ...
      {children}
      {note && <p className="text-xs text-zinc-400 italic border-t border-zinc-100 pt-2">
        {note}</p>}
    </div>
  );
}