//src/app/admin/(dashbaord)/finance/FinanceTable.tsx
"use client";

import { useState, useEffect } from "react";
import { useRouter } from "next/navigation";
import {
  verifyAndActivateRegistrationAction,
...
export default function FinanceTable({
  ...
  // Sync rows with incoming server props when revalidated
  useEffect(() => {
  ...
  const handleRecheckGateway = async () => {
    ...
    try {
      ...
      if (result.success && result.paymentStatus) {
        setRows((prev) =>
          prev.map((r) => (r.id === selected.id ? { ...r, paymentStatus: result.paymentStatus! } : r))
        );
        setSelected((prev) => (prev ? { ...prev, paymentStatus: result.paymentStatus! } : null));
      } else {
        alert(result.error ?? "Payment is still not detected by gateway.");
      }
    } catch (err: any) {
      alert(err?.message ?? "Failed to query gateway.");
    } finally {
      setIsSyncing(false);
    }
  };
  ...
  const handleManualMarkAsPaid = async () => {
    ...
    try {
      ...
      if (result.success) {
        setRows((prev) =>
          prev.map((r) => (r.id === selected.id ? { ...r, paymentStatus: "paid" } : r))
        );
          ...
          {rows.map((reg) => (
            ...
            >
              <td className="py-3 text-zinc-700">
                {String(reg.id ?? "").slice(0, 8).toUpperCase()}
              </td>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg p-8 text-zinc-900">
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              ...
              <div>
                ...
                <p className="border rounded px-3 py-2 font-medium">
                  ₱{selected.amount?.toLocaleString() ?? 0}
                </p>
              </div>
              <div className="col-span-2">
                    ...
                    <div className="flex gap-2">
                      {selected.paymentIntentId && (
                        <button
                          ...
                          disabled={isSyncing}
                          className="text-xs bg-blue-700 hover:bg-blue-800 text-white px-2.5 py-1 rounded transition-colors disabled:opacity-50"
                        >
                      ...
                      <button
                        type="button"
                        onClick={handleManualMarkAsPaid}
                        disabled={isUpdatingStatus}
                        className="text-xs bg-amber-600 hover:bg-amber-700 text-white px-2.5 py-1 rounded transition-colors disabled:opacity-50"
                      >
            ...
            <button
              ...
              className="w-full bg-green-800 hover:bg-green-900 transition-colors text-white rounded-lg py-3 font-medium disabled:opacity-50"
            >
              {isSubmitting
                ? "Processing..."
                : selected.paymentStatus !== "paid"
