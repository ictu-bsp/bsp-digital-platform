...
// Registrar/Activities admin table: lists every activity (published and
// unpublished), with a modal to post a new one and inline actions to
// toggle publish status or delete.
// NOTE: `createdBy` is not yet set on creation (needs the current admin's
// user id from the session) — schema allows null there, so this is safe
// for now but should be wired up later.
...
export default function ActivitiesTable({ initialActivities, councils }: Props) {
  ...
  // Converts a stored Date into the "YYYY-MM-DDTHH:mm" string that
  // <input type="datetime-local"> requires, so the edit form pre-fills
  // with the activity's existing values.
  ...
  const handleUpdate = async (event: React.FormEvent) => {
    ...
    const result = await updateActivityAction(editingActivityId, {
      ...
      registrationDeadline: editForm.registrationDeadline
        ? new Date(editForm.registrationDeadline)
        : null,
    ...
    setActivities((prev) =>
      prev.map((row) => (row.id === editingActivityId ? (result.data as ActivityRow) : row))
    );
  ...
  const handleTogglePublish = async (id: string, currentlyPublished: boolean) => {
    setPendingId(id);
    const result = await updateActivityAction(id, {
    ...
    setActivities((prev) =>
      prev.map((row) =>
        row.id === id ? { ...row, isPublished: !currentlyPublished } : row
      )
    );
  };
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-3 flex-1">
          ...
          <select
            ...
            className="border border-gray-300 rounded-lg px-3 py-2.5 text-sm text-zinc-900 focus:outline-none focus:ring-2 focus:ring-green-800"
          >
        ...
        <button
          ...
          className="text-sm font-bold px-4 py-2.5 rounded-lg text-white bg-green-800 hover:bg-green-900 transition-all shadow-sm whitespace-nowrap"
        >
          + Post New Activity
        </button>
      </div>
            ...
            {filteredActivities.map((row) => (
              <tr key={row.id} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="px-4 py-3">
                  {row.registrationFee ? `₱${row.registrationFee}` : "Free"}
                </td>
                ...
                <td className="px-4 py-3">
                  <div className="flex flex-col gap-2">
                    <button
                      ...
                      disabled={pendingId === row.id || deletingId === row.id}
                      className="text-xs font-bold px-3 py-1.5 rounded transition-all shadow-sm text-white bg-blue-700 hover:bg-blue-800 disabled:opacity-50"
                    >
                      Edit
                    </button>
                    ...
                    <button
                      ...
                      disabled={deletingId === row.id || pendingId === row.id}
          ...
          <div className="w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-2xl bg-white p-6 shadow-xl">
            ...
            <form onSubmit={handleCreate} className="flex flex-col gap-3">
              ...
              <button
                type="submit"
                disabled={submitting}
                className="rounded-lg bg-green-800 hover:bg-green-900 text-white font-bold py-2.5 mt-2 disabled:opacity-50"
              >
            ...
            <form onSubmit={handleUpdate} className="flex flex-col gap-3">
              ...
              <div className="flex gap-2">
                <label className="text-xs font-semibold text-zinc-600 w-1/2">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setEditForm({ ...editForm, category: e.target.value as ActivityCategory })
                    }
                    required
                  >
                    <option value="COUNCIL">Council</option>
                ...
                <label className="text-xs font-semibold text-zinc-600 w-1/2">
                  ...
                  <select
                    className="border rounded px-3 py-2 text-sm w-full mt-1"
                    value={editForm.councilId}
                    onChange={(e) => setEditForm({ ...editForm, councilId: e.target.value })}
                  >
                    <option value="">No specific council</option>
                    {councils.map((council) => (
                      <option key={council.id} value={council.id}>
              ...
              <label className="text-xs font-semibold text-zinc-600">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setEditForm({ ...editForm, registrationDeadline: e.target.value })
                  }
                />
              </label>
