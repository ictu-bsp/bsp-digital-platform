            ...
            {filteredRoster.length === 0 && (
              <tr>
                <td colSpan={7} className="px-4 py-6 text-center text-gray-400">
                  {roster.length === 0 ?
                  "No scouts have registered yet." :"No scouts match your search."}
                </td>
              </tr>
            )}
            {filteredRoster.map((row) => (
              <tr key={row.scoutId} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="px-4 py-3">
                  <span className={`text-xs font-bold px-2 py-1 rounded ${!row.isActive ?
                    "bg-red-100 text-red-700" : row.status === "ACTIVE" ?
                    "bg-green-100 text-green-700" : row.status === "PENDING" ?
                ...
                <td className="px-4 py-3">
                  <div className="flex flex-col gap-2">
                    <button onClick={() => handleToggle(row.scoutId, row.isActive)}
