...
export default async function AdminDashboardPage() {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex justify-end"><MonthRangePicker /></div>
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-white rounded-2xl shadow-sm p-6 min-h-[140px]">
          <p className="text-sm text-zinc-500 mb-1">Total Scouts</p>
          <p className="text-3xl font-bold text-green-800">{stats ? stats.totalScouts : "—"}</p>
        </div>
        <div className="bg-white rounded-2xl shadow-sm p-6 min-h-[140px]">
          <p className="text-sm text-zinc-500 mb-1">Total Councils</p>
          <p className="text-3xl font-bold text-green-800">{stats ? stats.totalCouncils : "—"}</p>
        </div>
      </div>
      <div className="bg-white rounded-2xl shadow-sm p-6 min-h-[200px]">
        ...
        <div className="grid grid-cols-2 gap-6">
          <div>
            <p className="text-sm font-semibold text-zinc-600 mb-2">Scouts by Sex</p>
            <div className="flex flex-col gap-2">
              {sexBreakdown.length === 0 && <p className="text-sm text-zinc-400">No data available.</p>}
              {sexBreakdown.map((row) => (
                <div key={row.sex ?? "unspecified"}>
                  ...
                  <div className="w-full bg-zinc-100 rounded-full h-2">
                    <div className="bg-green-800 h-2 rounded-full" style={{
                      width: `${Math.min(100, (row.value / totalScoutsForBars) * 100)}%` }} />
                  </div>
                </div>
              ))}
            </div>
          ...
          <div>
            <p className="text-sm font-semibold text-zinc-600 mb-2">Registrations by Status</p>
            <div className="flex flex-col gap-2">
              {statusBreakdown.length === 0 && <p className="text-sm text-zinc-400">No data available.</p>}
      ...
      <div className="bg-white rounded-2xl shadow-sm p-6 min-h-[140px]">
        ...
        <div className="grid grid-cols-2 gap-6">
          ...
          <div>
            <p className="text-sm font-semibold text-zinc-600 mb-2">By Region</p>
            <div className="flex flex-col gap-1">
              {regionBreakdown.length === 0 && <p className="text-sm text-zinc-400">No data available.</p>}
