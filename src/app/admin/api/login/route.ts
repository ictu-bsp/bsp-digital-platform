...
export async function POST(req: NextRequest) {
  try {
    ...
    // FIXED: Verify that the officer account belongs to the user's council
    // instead of matching user.id against createdBy (which is set by seeders).
    if (
      user.role === "COUNCIL_ADMIN" &&
      adminUser.councilId !== user.councilId
    ) {
      return NextResponse.json(
        { message: "This administrator account does not belong to your council." },
        { status: 403 }
      );
    }
