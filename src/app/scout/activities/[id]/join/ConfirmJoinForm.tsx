...
// Handles activity confirmation logic and renders join action controls
export default function ConfirmJoinForm({ activityId, activityTitle }: ConfirmJoinFormProps) {
  ...
  // Submits the activity registration request and displays the success overlay
  const handleJoin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setShowSuccess(true);
      } else setErrorMsg(result.error || "Failed to join activity.");
    } catch {
    ...
    <>
      ...
      <form onSubmit={handleJoin} className="mt-4">
        <button type="submit" disabled={isPending || showSuccess}
          className="w-full rounded-xl bg-emerald-700 px-4 py-3 text-center text-base font-semibold
