...
export default function ConfirmLeaveForm({
  activityId,
  activityTitle,
}: ConfirmLeaveFormProps) {
  ...
  const handleLeave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setErrorMsg(result.error || "Failed to leave activity.");
      }
    } catch {
    ...
    <>
      ...
      <form onSubmit={handleLeave} className="mt-5">
        <button
          ...
          className="w-full rounded-xl bg-red-600 px-4 py-3 text-base font-semibold text-white transition hover:bg-red-700 disabled:cursor-not-allowed disabled:opacity-60"
        >
