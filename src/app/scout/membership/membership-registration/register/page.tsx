...
export default function RegisterPage() {
  ...
  const handleRegionChange = (newRegionId: string) => {
    ...
    if (councils.find((c) => c.id === councilId)?.regionId !== newRegionId) setCouncilId("");
  };
    ...
    <div className="flex flex-col items-center justify-center py-10 px-6 bg-zinc-50 min-h-screen">
      <form onSubmit={onNext} className="w-full max-w-3xl bg-white rounded-2xl shadow-xl p-14 text-zinc-900 flex flex-col gap-5">
        ...
        <div className="relative">
          ...
          >
            <option value="" disabled className="text-zinc-400">
              {eligiblePositions === null ? "Loading..." : eligiblePositions.length === 0 ? "No scouting position available for your age" : "Scouting Position"}
            </option>
            {eligiblePositions?.map((position) => (
              <option key={position.value} value={position.value} className="text-zinc-900">
        ...
        {eligiblePositions?.length === 0 && (
          <p className="-mt-2 text-xs text-red-600">Based on your birthdate, you don't currently fall within any BSP Scouting Position age bracket (5–26 years old).</p>
        )}
