...
const BANK_CONFIG: Record<
  Bank,
  { type: "dob" | "brankas"; bankCode: string; label: string }
> = {
  bpi:        { type: "dob",     bankCode: "bpi",        label: "BPI" },
  unionbank:  { type: "dob",     bankCode: "ubp",         label: "UnionBank" },
  bdo:        { type: "brankas", bankCode: "bdo",         label: "BDO" },
  landbank:   { type: "brankas", bankCode: "landbank",    label: "LandBank" },
  metrobank:  { type: "brankas", bankCode: "metrobank",   label: "Metrobank" },
};

type OnlineBankingProps = {
  amount: number;
  description: string;
  bank: Bank;
};

export default function OnlineBanking({ amount, description, bank }: OnlineBankingProps) {
  const { type, bankCode, label } = BANK_CONFIG[bank];
      ...
      body: JSON.stringify({
        data: {
          attributes: {
            ...
            return_url: `${baseUrl}/payments/method/onlinebanking/return`,
          },
        },
      }),
    })
      ...
      <form onSubmit={onSubmit} className="flex flex-col gap-4">
        <div>
          ...
          <p className="text-sm text-zinc-500 mt-1">
            You'll be redirected to {label} online banking to authorize this payment.
          </p>
        </div>
