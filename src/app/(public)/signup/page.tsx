...
export default function SignUpPage() {
  ...
  const submitRegistration = async () => {
    ...
    if (!result.success) {
      setErrors(
        result.errors ?? (result.message ? { root: [result.message] } : {}),
      );
      return;
    }
    ...
    router.push(
      `/signup/verify?email=${encodeURIComponent(formData.email)}&age=${age}`,
    );
  };
        ...
        <div className="w-full max-w-md bg-white px-6 pb-8 pt-4 md:rounded-2xl md:border md:border-gray-100 md:shadow-sm">
          <div className="mb-6">
            ...
            <p className="mt-2 text-sm text-gray-500">
              Enter your information to receive a verification code. You'll
              create your password after verifying your email.
            </p>
            {role && (
              <div className="mt-3 flex items-center justify-between rounded-lg bg-green-50 px-3 py-2 text-sm text-green-900">
                <span>
                  Registering as:{" "}
                  <strong>
                    {role === "SCOUT" ? "Existing Scout" : "New to Scouting"}
                  </strong>
                </span>
                <button
                  type="button"
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex gap-3">
                ...
                <div className="w-[30%]">
                  <select
                    ...
                    onChange={handleInputChange}
                    className={`w-full rounded-lg border bg-white p-3 text-base text-gray-700 outline-none transition-all focus:ring-1 ${errors.suffix ? "border-red-500 focus:border-red-500 focus:ring-red-500" : "border-gray-300 focus:border-green-900 focus:ring-green-900"}`}
                  >
            ...
            <div>
              <input
                ...
                onChange={handleInputChange}
                className={`w-full rounded-lg border p-3 text-base outline-none transition-all focus:ring-1 placeholder:text-gray-400 disabled:cursor-not-allowed disabled:bg-gray-100 disabled:text-gray-400 ${errors.middleName ? "border-red-500 focus:border-red-500 focus:ring-red-500" : "border-gray-300 focus:border-green-900 focus:ring-green-900"}`}
              />
              {errors.middleName && (
                <p className="mt-1 pl-1 text-xs text-red-600">
                  {errors.middleName[0]}
              ...
              <div className="flex gap-3">
                <div className="w-[65%]">
                  <input
                    ...
                    className={`w-full rounded-lg border p-3 text-base outline-none transition-all focus:ring-1 ${
                      !formData.birthdate ? "text-gray-400" : "text-gray-900"
                    } ${
                      errors.birthdate
                ...
                <div className="w-[35%]">
                  <select
                    ...
                    value={formData.sex}
                    onChange={handleInputChange}
                    className={`w-full rounded-lg border bg-white p-3 text-base text-gray-700 outline-none transition-all focus:ring-1 ${errors.sex ? "border-red-500 focus:border-red-500 focus:ring-red-500" : "border-gray-300 focus:border-green-900 focus:ring-green-900"}`}
                  >
                    <option value="" disabled hidden>
                      Sex
                    </option>
      ...
      {showPolicyModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4 py-6">
          <div className="flex max-h-[90vh] w-full max-w-lg flex-col rounded-2xl bg-white p-6 shadow-xl">
            <h2 className="text-xl font-bold text-green-900">
              Safe from Harm & Privacy Policy
            </h2>
            <p className="mt-1 text-xs text-gray-500">
              Please read through the policy below before proceeding.
            </p>
            <div
              ref={policyScrollRef}
              onScroll={handlePolicyScroll}
              className="mt-4 max-h-[350px] space-y-3 overflow-y-auto rounded-lg border border-gray-200 bg-gray-50 p-4 text-xs leading-relaxed text-gray-700"
            >
              <h3 className="text-sm font-bold text-gray-900">
                1. Safe from Harm Commitment
              </h3>
              <p>
                eScout is committed to preventing all forms of abuse,
                harassment, neglect, and exploitation within Scouting
                activities. Every participant has the right to a safe,
              ...
              <p>
                Users agree to interact responsibly and refrain from any
                behavior that compromises physical, emotional, or digital
                safety. Any misconduct or suspected breach of safety guidelines
                will result in account review or immediate suspension.
              </p>
              <h3 className="text-sm font-bold text-gray-900">
                3. Privacy & Data Handling
              </h3>
              <p>
                We handle personal information (names, birthdates, and emails)
                strictly for account management, membership verification, and
                safeguarding purposes. Your data is protected in accordance with
              ...
              <p>
                Applicants under 18 years of age require parental or legal
                guardian notification and approval during account verification.
              </p>
            </div>
            {!hasScrolledToBottom && (
            ...
            <div className="mt-4 flex items-start gap-2.5">
              <input
                ...
                className="mt-0.5 h-4 w-4 rounded border-gray-300 accent-green-900 focus:ring-green-900 disabled:cursor-not-allowed"
              />
              <label
                htmlFor="acceptPolicy"
                className={`select-none text-xs ${hasScrolledToBottom ? "text-gray-700" : "text-gray-400"}`}
              >
                I have read, understood, and agree to the{" "}
                <strong>Safe from Harm Policy</strong> and{" "}
            ...
            <div className="mt-6 flex justify-end gap-3">
              ...
              <button
                ...
                disabled={!hasAcceptedPolicy || isSubmitting}
                className="rounded-lg bg-green-900 px-5 py-2 text-sm font-bold text-white transition hover:bg-green-950 disabled:cursor-not-allowed disabled:opacity-50"
              >
