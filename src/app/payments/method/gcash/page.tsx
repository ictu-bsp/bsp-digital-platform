"use client";
// src/app/payments/method/gcash/page.tsx
// Reached after the user picks "E-Wallets" -> "GCash" on /payments/method.
// Reads the amount + description that register/page.tsx saved to
// localStorage, then hands them to the existing GCash component.

import { useEffect, useState } from "react";
import { useRouter } from "next/navigation";
import GCash from "../../components/GCash";

export default function GCashMethodPage() {
  ...
  return (
    <div className="flex flex-col items-center justify-center py-10 px-6 bg-zinc-50 min-h-screen">
      <div className="w-full max-w-3xl bg-white rounded-2xl shadow-xl p-14 text-zinc-900">
        ...
        <h1 className="text-4xl font-bold text-green-800 mb-2">GCash</h1>
        <p className="text-zinc-600 text-lg mb-8">Amount to pay: ₱{amount}</p>

        <GCash amount={amount} description={description} />
      </div>
    </div>
  );
}