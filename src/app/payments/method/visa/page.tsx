"use client";
// src/app/payments/method/visa/page.tsx
// Reached after the user picks "Credit / Debit Card" -> "Visa" on /payments/method.
// Reads the amount + description that register/page.tsx saved to
// localStorage, then hands them to the Visa component.

import { useEffect, useState } from "react";
import { useRouter } from "next/navigation";
import Visa from "../../components/Visa";

export default function VisaMethodPage() {
  ...
  return (
    <div className="flex flex-col items-center justify-center py-10 px-6 bg-zinc-50 min-h-screen">
      <div className="w-full max-w-3xl bg-white rounded-2xl shadow-xl p-14 text-zinc-900">
        ...
        <h1 className="text-4xl font-bold text-green-800 mb-2">Visa</h1>
        <p className="text-zinc-600 text-lg mb-8">Amount to pay: ₱{amount}</p>

        <Visa amount={amount} description={description} />
      </div>
    </div>
  );
}