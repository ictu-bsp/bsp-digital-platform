...
import { z } from "zod";
import { emailSchema, nameSchema, birthdateSchema, suffixSchema } from "./shared";

// Helper to normalize empty string inputs to undefined
const optionalString = <T extends z.ZodTypeAny>(schema: T) => schema.optional().or(z.literal("")).transform((val) => (val === "" || val === null ? undefined : val));

// Schema for initial signup without parent email validation
